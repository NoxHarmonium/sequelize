  ...
  */
  ...
  /*
    ...
    Options:
      ...
      - tableSample -> A hash with parameters for table sampling
  */
  ...
  selectQuery: function(tableName, options, model) {
    ...
    // Add TABLESAMPLE to sub or main query
    if (options.tableSample) {
      var method = options.tableSample.method;
      var percentage = options.tableSample.percentage;
      var repeatable = options.tableSample.repeatable;
      ...
      if (!_.includes([
        'SYSTEM',
        'BERNOULLI',
        'SYSTEM_ROWS', // Extension for POSTGRES
      ], method.toUpperCase())) {
        throw new Error(util.format('Method must be \'SYSTEM\' or \'BERNOULLI\', \'%s\' given', method));
      }

      if (typeof percentage !== 'number') {
        throw new Error(util.format('Percentage property is required, \'%s\' given', percentage));
      }

      // SYSTEM_ROWS is not a percentage so it doesn't have to be validated here
      if (_.includes([
        'SYSTEM',
        'BERNOULLI',
      ], method.toUpperCase()) &&
        percentage < 0 || percentage > 100) {
        throw new Error(util.format('Percentage must be a number between 0 and 100, \'%s\' given', percentage));
      }

      mainQueryItems.push(' TABLESAMPLE ' + method + '(' + percentage + ')');

      if (typeof repeatable === 'number') {
        mainQueryItems.push(' REPEATABLE(' + repeatable + ')');
      }
    }
